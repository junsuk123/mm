...
. "$SCRIPT_DIR/providers/restaurant_provider.sh"
. "$SCRIPT_DIR/providers/naver_restaurant_provider.sh"
...
usage() {
  cat <<'EOF'
Usage:
  sh scripts/recommend.sh --collect-session [--provider naver] [--location 세종대학교]
  sh scripts/recommend.sh --demo [--without-mobile-responses] [--mobile-session-id ID] [--provider naver] [--location 세종대학교]
  sh scripts/recommend.sh --session-file session.json [--json-output] [--provider naver] [--location 세종대학교]
  sh scripts/recommend.sh --user-id U01 [--provider naver] [--location 세종대학교]
...
build_group_assignments_file() {
  ...
  sh "$SCRIPT_DIR/grouping_cli.sh" --session-file "$session_file" --group-count "$group_count" > "$assignments_file"
}
...
build_group_exclusions_file_from_members() {
  session_file=$1
  members_json=$2
  exclusions_file=$3
  ...
  jq -c \
    --argjson members "$members_json" '
      [
        .participants[]
        | select(.user_id as $user_id | ($members | index($user_id)))
...
recommend_for_single_user() {
  ...
  exclusions_file=$(mktemp "${TMPDIR:-/tmp}/recommend-exclusions.XXXXXX")
...
recommend_for_groups() {
  ...
  web_step \
    ...
    "jq 기반 CLI가 참가자의 음식 선호 유사도를 계산해 그룹 구성을 완료했습니다."
  ...
  jq -c '.groups[]' "$assignments_file" | while IFS= read -r group_json; do
    ...
    member_names_json=$(jq -c \
      --argjson members "$members_json" '
        [
          $members[] as $member_id
          | (
              .participants[]
              | select(.user_id == $member_id)
              | (.display_name // .original_user_id // .user_id)
            ) // $member_id
        ]
      ' "$session_file")
    ...
    group_json=$(jq -n \
      ...
      --argjson members "$members_json" \
      --argjson member_names "$member_names_json" \
      --argjson excluded_restaurant_count "$excluded_restaurant_count" \
      --slurpfile recommendations "$recommendations_file" \
      '{
        group_id: $group_id,
        members: $members,
        member_names: $member_names,
        excluded_restaurant_count: $excluded_restaurant_count,
...
print_group_summary() {
  ...
  jq -r '
    ...
    | ((.member_names // .members) | join(", ")) as $members
    | .recommendations[0] as $r
    | if $r == null then
...
open_visualization_report() {
  result_file=$1
  visualizer_script="$PROJECT_ROOT/src/recommendation_visualizer.py"
  ...
  mkdir -p "$output_dir"
  if ! MM_VISUALIZER_SKIP_LIVE=1 PYTHONPATH="$PROJECT_ROOT/src${PYTHONPATH:+:$PYTHONPATH}" python3 "$visualizer_script" --input "$result_file" --output "$output_html"; then
