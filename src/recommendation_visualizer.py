...
def participant_display_name(participant: dict[str, Any]) -> str:
    return str(
        ...
        or participant.get("original_user_id")
...
def group_member_names(
    ...
    explicit_names = group.get("member_names")
    if isinstance(explicit_names, list) and explicit_names:
        return [str(name) for name in explicit_names]
    return [
        participant_display_name(members_by_id.get(member_id, {"user_id": member_id}))
...
def render_participant_section(participant: dict[str, Any]) -> str:
    ...
    <article class="panel">
        <div class="panel-head">
            <h3>{html.escape(participant_display_name(participant))}</h3>
...
def render_recommendation_network(report: dict[str, Any]) -> str:
    ...
    svg_parts = [
        ...
        '<text x="930" y="30" class="network-axis">Top Restaurant</text>',
    ]

    for group_index, group in enumerate(groups):
        row_y = top_pad + group_index * row_height
        center_y = row_y + 92
        members = group_member_names(group, members_by_id)
        recommendations = list(group.get("recommendations", []))[:1]
        participant_gap = 42
        restaurant_gap = 54
...
def render_group_section(group: dict[str, Any], report: dict[str, Any], members_by_id: dict[str, dict[str, Any]]) -> str:
    ...
    members_text = html.escape(", ".join(group_member_names(group, members_by_id)))
...
def render_flow(report: dict[str, Any]) -> str:
    ...
    <section class="flow">
      ...
      <div class="flow-arrow">→</div>
      <div class="flow-step"><div class="flow-number">4</div><div class="flow-title">추천 식당 선정</div><div class="muted">점수 1위 한 곳</div></div>
    </section>
    """
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--input", required=True, help="Path to the JSON report produced by scripts/recommend.sh")
