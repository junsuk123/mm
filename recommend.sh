...
collect_personal_preferences_json() {
  ...
  dislike_json=$(collect_category_triplet_json "dislike")
  recent_json=$(collect_category_triplet_json "recent")

  jq -n \
    --arg user_id "$user_id" \
    --argjson like "$like_json" \
    --argjson dislike "$dislike_json" \
...
build_profile_file_from_user() {
  ...
  jq -c --arg user_id "$user_id" '
    ...
    | .preferences
    | {like: (.like | normalize_levels), dislike: ((.dislike // {}) | normalize_levels), recent: (.recent | normalize_levels)}
    | {
        like_high: (.like.high | map(leaf)),
        like_low: (.like.low | map(leaf)),
        dislike_high: (.dislike.high | map(leaf)),
        dislike_low: (.dislike.low | map(leaf)),
...
run_demo_session() {
  ...
  jq -c '.participants[]' "$SESSION_JSON_FILE" | while IFS= read -r participant_json; do
    ...
    emit_demo_category_group_trace dislike "$participant_json"
...
build_group_profile_file() {
  ...
  jq -c \
    ...
    --argjson group_count "$group_count" '
      ...
      | map(.value.preferences | {like: (.like | normalize_levels), dislike: ((.dislike // {}) | normalize_levels), recent: (.recent | normalize_levels)})
      | reduce .[] as $p (
          {like_high: [], like_low: [], dislike_high: [], dislike_low: [], recent: []};
          .like_high += ($p.like.high | map(leaf))
          | .like_low += ($p.like.low | map(leaf))
          | .dislike_high += ($p.dislike.high | map(leaf))
          | .dislike_low += ($p.dislike.low | map(leaf))
...
build_group_profile_file_from_members() {
  ...
  jq -c \
    --argjson members "$members_json" '
      ...
      | map(.preferences | {like: (.like | normalize_levels), dislike: ((.dislike // {}) | normalize_levels), recent: (.recent | normalize_levels)})
      | reduce .[] as $p (
          {like_high: [], like_low: [], dislike_high: [], dislike_low: [], recent: []};
          .like_high += ($p.like.high | map(leaf))
          | .like_low += ($p.like.low | map(leaf))
...
recommend_from_profile_file() {
  ...
  jq -s --slurpfile profile "$profile_file" '
    ...
    def score($r; $p):
      ...
      - (if has($p.dislike_high; $r.category) then 0.8 else 0 end)
      - (if has($p.dislike_low; $r.food) then 1.0 else 0 end);
    def reason($r; $p):
      [
        ...
        (if has($p.dislike_high; $r.category) then "disliked category \($r.category) was penalized" else empty end),
        (if has($p.dislike_low; $r.food) then "disliked food \($r.food) was strongly penalized" else empty end)
      ] as $parts
