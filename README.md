...
3. 세션에서 사용할 그룹 수를 선택합니다. 이후 그룹화는 세션 생성 시 선택한 수를 사용합니다.
4. 식당 검색 provider는 기본적으로 네이버 지역 검색 API(`naver`)를 사용합니다. API 없이 테스트할 때만 `mock`을 선택합니다.
5. 샘플 참가자 10명은 항상 포함됩니다. `이번 세션 참여자 선택`에서 추가할 실제 사용자 폴더를 누릅니다.
6. 모바일 접속 주소를 확인하고 `모바일 접수 시작`을 누릅니다.
7. 샘플 참가자 10명과 선택된 실제 사용자가 해당 세션 참가자로 복사되고 QR 코드와 모바일 접속 링크가 표시됩니다.
8. QR로 새로 응답한 참가자는 현재 세션과 영구 사용자 폴더에 함께 추가됩니다.
9. 접수를 끝낼 때 `마감 및 실행`을 누르면 현재 세션에 포함된 참가자만 선택한 그룹 수로 나누고 그룹별 식당 한 곳을 추천합니다.
...
새 세션은 샘플 참가자 10명을 기본으로 포함하고, 저장된 실제 사용자 중 이번에 함께 추천받을 사람을 추가하는 임시 묶음입니다. 세션을 시작할 때 샘플과 선택한 사용자의 최신 프로필을 세션 참가자 목록으로 복사합니다. 세션이 접수 중일 때 관리자 화면에서 저장 사용자 폴더 선택을 변경하면 현재 세션 참가자 목록에도 즉시 반영됩니다. 아무 폴더도 선택하지 않아도 샘플 10명은 포함되며, QR로 새로 참여한 사용자도 저장 폴더와 현재 세션 참가자 표시에 자동으로 추가됩니다.
...
4. 1명만 남은 그룹이 있으면 인원이 가장 많은 그룹에서 한 명을 재배치합니다.

그룹 수는 모든 그룹에 최소 2명이 들어갈 수 있는 범위에서만 선택할 수 있습니다. 따라서 선호 카테고리 종류가 그룹 수보다 많아도 fallback으로 떨어지지 않고, 목표 그룹 수 안에서 비슷한 취향끼리 최대한 보존하며 1인 그룹을 만들지 않습니다.
...
최근 음식 필터는 실제 음식명이나 값이 있는 세부분류가 일치할 때만 적용합니다. 네이버 API가 짧은 시간에 너무 많은 요청을 받아 `429 Too Many Requests`를 반환하면 잠시 기다렸다가 자동으로 재시도하며, 음식별 검색 사이에도 기본 0.25초 간격을 둡니다.

거리·리뷰 필터 때문에 후보가 0개가 되면 해당 두 필터만 자동 완화합니다. 그래도 후보가 없으면 주변 `음식점`, `맛집`으로 검색 범위를 넓혀 그룹마다 식당 한 곳이 나오도록 재시도합니다. 최근 먹은 음식, 사용자의 `안갈래요`, 다른 그룹이 이미 선택한 식당은 이 과정에서도 계속 제외됩니다.

한 세션 안에서는 앞 그룹이 선택한 식당 ID를 다음 그룹의 제외 목록에 누적합니다. 따라서 서로 다른 그룹에 같은 식당이 중복 추천되지 않습니다.
