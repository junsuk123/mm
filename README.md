...
4. `모바일 QR 수집 사용`을 체크합니다.
5. `새 세션 시작`을 누르면 QR 코드와 모바일 접속 링크가 표시됩니다.
6. 참가자는 QR을 스캔하고 모바일 설문에 응답합니다.
7. 참가자 목록에는 `dataset/demo_session.json`의 기본 참가자 뒤에 QR 참가자가 계속 추가됩니다.
8. 접수를 끝낼 때 `마감 및 실행`을 누르면 현재 참가자 전체로 추천을 계산합니다.
...
그룹 생성은 `grouping_utils.py`에서 처리합니다.

1. 선호 항목이 충분히 겹치면 유사도 기준으로 그룹을 만듭니다.
2. 유사도 그룹이 어렵다면 가장 많이 공통된 항목을 기준으로 fallback 그룹을 만듭니다.

이 방식으로 참가자 분포가 고르지 않아도 그룹 추천이 계속 진행됩니다.
